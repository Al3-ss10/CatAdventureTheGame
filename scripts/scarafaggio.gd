...
func Morte():
	...
	morte = true
	morto.emit() # Emette il segnale quando il nemico muore
...
func _on_salto_body_entered(body: Node2D) -> void:
	if fase_salto != FaseSalto.NESSUNA or in_cooldown_salto:
		return
	if body.is_in_group("protagonista") and morte==false:
		giocatore = body
		_inizia_salto()
...
func _inizia_planata() -> void:
	...
	animated_sprite_2d.play("salto") # Usa "salto" anche durante la planata
